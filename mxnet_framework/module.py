...
class LRTestModule(Module):

    def __init__(self, *args, **kwargs):
        super(LRTestModule, self).__init__(*args, **kwargs)

    def fit(self, train_data, eval_data=None, eval_metric='acc',
            epoch_end_callback=None, batch_end_callback=None, kvstore='local',
            optimizer='sgd', optimizer_params=(('learning_rate', 0.01),),
            eval_end_callback=None, display=10,
            eval_batch_end_callback=None, initializer=Uniform(0.01),
            arg_params=None, aux_params=None, allow_missing=False,
            force_rebind=False, force_init=False, begin_epoch=0, num_epoch=None,
            validation_metric=None, monitor=None):
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                # evaluation on validation set
                if eval_data and nbatch % display == 0:
                    eval_res = self.score(eval_data, validation_metric,
                                          score_end_callback=eval_end_callback,
                                          batch_end_callback=eval_batch_end_callback, epoch=epoch)
                    cur_lr = self._optimizer.learning_rate
                    for name, val in eval_res:
                        self.logger.info('Epoch[%d] Batch[%d] LR[%f] Validation-%s=%f', epoch, nbatch, cur_lr, name, val)

                    result.append((epoch, nbatch, cur_lr, eval_res))
